...
{
    int pro_name;
    int arrivalTime, waiturnTime, turnaroundTime, priority, burstTimecopy;
}queue1[10],queue2[10];
int main()
{
	struct process temp;
	...
	//fflush(stdin);
        ...
        scanf("%d",&queue1[i].arrivalTime);
        printf("Enter Burst Time:-");
        scanf("%d",&queue1[i].burstTimecopy);
        queue1[i].burstTimecopy=queue1[i].burstTimecopy;
	...
	while(count!=t1)
	{
		...
		if(queue1[i].arrivalTime<=time)
		{
			for(j=i+1;j<totalProcess;j++)
			{
				if(queue1[j].arrivalTime==time && queue1[j].priority<queue1[i].priority)//pr<
				{
					queue2[pf2]=queue1[i];
						pf2++;
			...
			if(queue1[i].burstTimecopy==0)
			{
				...
				queue1[i].waiturnTime=queue1[i].turnaroundTime-queue1[i].burstTimecopy;
				printf("%d\t|\t%d\t|\t%d\n",queue1[i].pro_name,queue1[i].turnaroundTime,queue1[i].waiturnTime);
				wait_time+=time-queue1[i].waiturnTime; 
	...
	else if(queue2[count].burstTimecopy>0) 
	{ 
	...
	{ 
		totalProcess2--; 
		queue2[count].turnaroundTime=time-queue2[count].arrivalTime;
			queue2[count].waiturnTime=queue2[count].turnaroundTime-queue2[count].burstTimecopy; 
			printf("%d\t|\t%d\t|\t%d\n",queue2[count].pro_name,queue2[count].turnaroundTime,queue2[count].waiturnTime); 
		turnaround_time+=time-queue2[count].arrivalTime; 
		wait_time+=time-queue2[count].arrivalTime-queue2[count].burstTimecopy;
